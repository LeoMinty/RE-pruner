...
# --- 4. 优化器 ---
optimizer = torch.optim.AdamW(pruned_model.parameters(), lr=FINETUNE_LR) 
...
def validate(model, loader, criterion, device):
    ...
    for images, labels in pbar:
        ...
        pbar.set_postfix({"Loss": loss.item(), "Top-1": (100 * correct_1.item() / total)})
...
for epoch in range(FINETUNE_EPOCHS):
    ...
    val_loss, val_acc1 = validate(pruned_model, val_loader, criterion, device)
    print(f"Epoch {epoch+1}: Top-1 Acc: {val_acc1:.2f}%")
...
print(f"微调完成！Rate {RATE} 最终最佳精度: {best_acc1:.2f}%")