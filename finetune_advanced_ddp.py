...
def main():
    ...
    # [策略 4] 使用 timm 的 create_transform (包含 RandAugment)
    # is_training=True 会自动应用 RandAugment, ColorJitter 等
    train_transform = create_transform(
        input_size=224,
        is_training=True,
        color_jitter=0.4,
        auto_augment='rand-m9-mstd0.5-inc1', # 强力增广
    ...
    model = PrunedVisionTransformer(
        ...
        drop_path_rate=0.1, # 加上 DropPath 防止过拟合
        norm_layer=partial(nn.LayerNorm, eps=1e-6)
    )
    ...
    for epoch in range(FINETUNE_EPOCHS):
        ...
        # 2. 验证 EMA 模型 (只在主进程有EMA，但DDP验证需要同步，这里简化处理：只打印主进程的EMA结果)
        # 注意：严谨的做法是广播 EMA 权重，但这里为了速度，我们只参考主进程的 EMA 性能
        ...
        if is_master:
            ...
            if save_acc > best_acc1:
                best_acc1 = save_acc
                torch.save(model.module.state_dict(), "pruned_finetuned_best.pth")
                if model_ema:
                    torch.save(model_ema.module.state_dict(), "pruned_finetuned_best_ema.pth")
                print(">>> 保存新的最佳模型 <<<")
