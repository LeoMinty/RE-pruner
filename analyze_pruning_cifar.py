...
parser.add_argument('--pruning_rate', type=float, required=True, help='Target pruning rate (e.g., 0.4)')
...
print(f"--- 正在分析 CIFAR-10 模型 (Rate: {RATE}) ---")
print("--- 正在重建物理剪枝模型结构 ---")
...
if not os.path.exists(PRUNED_MODEL_PATH):
    raise FileNotFoundError(f"模型文件 {PRUNED_MODEL_PATH} 不存在。请先运行 prune_model_cifar.py。")
pruned_model.load_state_dict(torch.load(PRUNED_MODEL_PATH, map_location=device))
pruned_model.eval()

# --- 2. 加载原始的、未剪枝的模型 ---
# 注意：这里需要创建一个标准的 DeiT 结构，类别为 10
...
if layer_pruning_rates:
    ...
    print(f"\n--- 平均加权参数剪枝率: {avg_r:.4f} ---")
    outfile_name= f'layer_pruning_rates_cifar10_r{int(avg_r*100)}.png' # <--- 修改文件名
    ...
    plt.title(f'Layer-wise Weighted Pruning Rates (Avg = {avg_r:.2%}) - CIFAR10')
...
try:
    ...
    print(f"\n[Pruned] RE-Pruner Model (CIFAR-10):")
